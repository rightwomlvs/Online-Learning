...
// --- Report Modal ---
const ReportModal = ({ state, onClose, formatSeconds }: { state: any, onClose: () => void, formatSeconds: any }) => {
...
const App = () => {
  ...
  const formatSeconds = (ts: number) => {
    const m = Math.floor(ts / 60);
    const s = ts % 60;
    return `${m.toString().padStart(2, '0')}:${s.toString().padStart(2, '0')}`;
  };

  const addLog = useCallback((type: string, name: string, value?: string, duration?: number) => {
    ...
    if (state.isSessionActive) {
      tickerRef.current = setInterval(() => {
        ...
        setState(prev => {
          ...
          if (prev.activeMode) newModeTimes[prev.activeMode as keyof typeof newModeTimes] += 1;
          prev.activeActions.forEach(id => newActionTimes[id as keyof typeof newActionTimes] += 1);
  ...
  const handleActionInteraction = (actionId: string, type: 'start' | 'end') => {
    ...
    if (type === 'start') {
      ...
      if (longPressTimerRef.current[actionId]) {
        ...
        setState(prev => ({ 
          ...prev, 
          actionCounts: { ...prev.actionCounts, [actionId as keyof typeof prev.actionCounts]: prev.actionCounts[actionId as keyof typeof prev.actionCounts] + 1 }
        }));
        ...
        <section className="lg:col-span-3 space-y-6">
          ...
          <div className="grid gap-4">
            {MODES.map(mode => (
              <button 
                key={mode.id}
                onClick={() => state.isSessionActive && setState(p => ({...p, activeMode: p.activeMode === mode.id ? null : mode.id as any, lastInteraction: Date.now()}))}
        ...
        <section className="lg:col-span-6 space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-5">
            {ACTIONS.map(action => (
              ...
              >
                <div className="text-4xl font-bold mb-1">{state.actionCounts[action.id as keyof typeof state.actionCounts]}</div>
                <span className="text-[11px] font-bold text-amber-500/40">{action.label}</span>
                {state.actionTimes[action.id as keyof typeof state.actionTimes] > 0 && <div className="text-[9px] font-mono mt-2 opacity-30">{formatSeconds(state.actionTimes[action.id as keyof typeof state.actionTimes])}</div>}
              </button>
            ))}
          </div>
        </section>
        ...
        <div className="flex gap-4 items-center">
          ...
          <div className="grid grid-cols-3 gap-2 w-48 h-10">
            {(['high', 'med', 'low']).map(l => (
              <button 
                key={l}
                onClick={() => state.isSessionActive && setState(p => ({...p, engagement: l as any, lastInteraction: Date.now()}))}
